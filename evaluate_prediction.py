...
def main(opt):
    ...
    #if 
    import torch
    src_file = opt.src_file_path
    dataset_name = src_file.split('../data/testsets/')[0].split('/test_src.txt')[0]
    outfile = opt.exp_path + '/score_dict.bin'
    torch.save( score_dict, open(outfile, 'wb'))
    if opt.export_filtered_pred:
    ...
    if opt.tune_f1_v:
        ...
        result_txt_str += ('Total #unique predictions: %d/%d, dup ratio %.3f\n' % (total_num_unique_predictions,
                       total_num_predictions,( total_num_predictions - total_num_unique_predictions) / total_num_predictions))
